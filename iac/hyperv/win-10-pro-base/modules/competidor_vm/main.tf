...
# Output (Retorna o IP alocado dinamicamente)
output "real_ip_address" {
  // CORREÇÃO FINAL: Usamos 'ip_addresses' (plural) DENTRO do adaptador de rede [0]
  value = hyperv_machine_instance.competitor_host.network_adaptors[0].ip_addresses[0] 
  description = "O endereço IP real (dinâmico via DHCP) detectado pelo Hyper-V provider (lido da interface de rede)."
}